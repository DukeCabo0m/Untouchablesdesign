...
export function AlbumDetailPage() {
  ...
  // Determine album type label for breadcrumb
  const albumTypeLabel = album.type === 'studio' ? 'STUDIO' : 
                         album.type === 'live' ? 'LIVE' : 
                         album.type === 'compilation' ? 'COMPILATIONS' : 'SINGLES';
  ...
  return (
    <div className="min-h-screen">
      ...
      <PageHeader
        title={album.title}
        description={`Album ${album.type === 'studio' ? 'studio' : album.type} · ${album.year}<br />${album.tracks.length} pistes · ${album.producer || 'Korn'}`}
        backgroundImage={album.cover}
        breadcrumbs={[
          { label: 'ACCUEIL', path: '/' },
          { label: 'DISCOGRAPHIE', path: '/discography' },
          { label: albumTypeLabel, path: `/discography/${album.type}` },
