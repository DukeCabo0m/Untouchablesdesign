...
import { articlesApi, usersApi, categoriesApi } from '@/app/utils/api';
...
export function CategoryPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        setIsLoading(true);
        console.log('[CategoryPage] Loading data for category:', slug);
        
        // Load all data
        const [categoriesData, articlesData, usersData] = await Promise.all([
        ...
        const filteredArticles = articlesData
          .filter((a: any) => a.isPublished && a.category === slug)
        ...
        console.error('[CategoryPage] Failed to load data:', err);
      ...
      <div className="px-4 pb-24 bg-[#0A0A0A]">
        <div className="max-w-7xl mx-auto">
          ...
          {articles.length > 0 ? (
                  ...
                  >
                    <div className="bg-[#0A0A0A] border-2 border-[#E0E0E0]/20 hover:border-[#8B0000] hover:bg-[#1a1a1a] transition-all overflow-hidden h-full flex flex-col">
                      ...
                      <div className="aspect-[4/3] overflow-hidden relative">
                        ...
                        {/* Category badge */}
                        <div 
                          className="absolute top-4 left-4 px-4 py-2 uppercase text-xs font-black text-[#FFFFFF]"
                          style={{ backgroundColor: category.color }}
                        >
                          {category.name}
                        </div>
            ...
            >
              <Layers size={48} className="text-[#8B0000]/30 mx-auto mb-4" />
