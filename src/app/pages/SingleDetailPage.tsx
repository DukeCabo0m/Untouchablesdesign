...
import { singlesApi, commentsApi, usersApi } from '@/app/utils/api';
...
export function SingleDetailPage() {
  ...
  // Load single and comments from backend
  useEffect(() => {
    async function loadSingle() {
      ...
      try {
        setIsLoading(true);
        console.log('[SingleDetailPage] Loading single:', slug);
        
        // Get all singles and find by slug
        const singles = await singlesApi.getAll();
        const foundSingle = singles.find((s: any) => s.slug === slug);
        
        if (!foundSingle) {
          setError('Single non trouvé');
          setIsLoading(false);
          return;
        }
        
        console.log('[SingleDetailPage] Found single:', foundSingle);
        
        // Load comments in parallel
        const commentsData = await commentsApi.getByEntity('single', foundSingle.id).catch(() => []);
        
        setSingle(foundSingle);
        setComments(commentsData.filter((c: any) => c.isApproved));
        setError(null);
      } catch (err) {
        console.error('[SingleDetailPage] Failed to load single:', err);
        setError('Impossible de charger le single');
      } finally {
        setIsLoading(false);
      }
    }
  ...
  // Create streaming links object for compatibility
  const streamingLinks = single ? {
    spotify: single.spotifyUrl || single.streamingLinks?.spotify || '#',
    appleMusic: single.appleMusicUrl || single.streamingLinks?.appleMusic || '#',
    youtube: single.youtubeUrl || single.streamingLinks?.youtube || '#'
  ...
  return (
    <div className="min-h-screen py-32 px-4">
      <div className="max-w-6xl mx-auto">
        {/* Back Button */}
        <Link 
          to={`/discography/album/${single.albumSlug}`}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-12 mb-20">
          ...
          >
            <div className="relative overflow-hidden border-4 border-[#8B0000] aspect-square mb-8">
              <img
                src={single.coverImage || single.cover}
                alt={single.title}
            ...
            <div className="p-6 bg-[#0A0A0A] border-2 border-[#8B0000]">
              ...
              <div className="space-y-3">
                <a
                  ...
                  className="flex items-center gap-3 p-3 bg-[#1DB954]/20 border border-[#1DB954]/40 hover:bg-[#1DB954]/40 transition-colors cursor-pointer group"
                >
                ...
                <a
                  ...
                  className="flex items-center gap-3 p-3 bg-[#FA243C]/20 border border-[#FA243C]/40 hover:bg-[#FA243C]/40 transition-colors cursor-pointer group"
                >
                ...
                <a
                  ...
                  className="flex items-center justify-center gap-3 px-8 py-4 bg-[#FF0000]/20 border-2 border-[#FF0000] hover:bg-[#FF0000]/40 transition-colors cursor-pointer group w-full"
                >
          ...
          >
            <div className="mb-6">
              ...
              <div className="flex items-center gap-4 mb-6">
                ...
                <Link
                  to={`/discography/album/${single.albumSlug}`}
                  className="font-black text-lg text-[#E0E0E0] hover:text-[#8B0000] transition-colors cursor-pointer uppercase"
                >
                  {single.album}
                </Link>
              </div>
