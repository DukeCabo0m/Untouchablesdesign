...
export function UpcomingConcertsPage() {
  ...
  useEffect(() => {
    async function loadConcerts() {
      try {
        setIsLoading(true);
        console.log('[UpcomingConcertsPage] Loading concerts from backend...');
        
        const data = await concertsApi.getAll();
        console.log('[UpcomingConcertsPage] Received concerts:', data);
        
        // Filter upcoming concerts and sort by date
        const now = new Date();
        const upcomingConcerts = data
          .filter((c: any) => new Date(c.date) >= now)
          .sort((a: any, b: any) => new Date(a.date).getTime() - new Date(b.date).getTime());
        
        setConcerts(upcomingConcerts);
        setError(null);
      } catch (err) {
        console.error('[UpcomingConcertsPage] Failed to load concerts:', err);
        setError('Impossible de charger les concerts à venir');
        setConcerts([]);
      } finally {
        setIsLoading(false);
      }
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            ...
            <div className="space-y-4">
              {isLoading ? (
                <div className="text-center text-[#E0E0E0] font-mono text-sm">
